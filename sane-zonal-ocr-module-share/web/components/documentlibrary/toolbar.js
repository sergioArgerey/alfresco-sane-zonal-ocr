...
(function()
{
   ...
   YAHOO.lang.augmentObject(Alfresco.DocListToolbar.prototype,
   {
	...
	// Scan button: user needs "Scan" access @MICAMBIO
         this.widgets.scanner = Alfresco.util.createYUIButton(this, "scanner-button", this.onToolbarActionScan,
         {
            disabled: true,
            value: "CreateChildren"
         });
         this.dynamicControls.push(this.widgets.scanner);
      ...
      onToolbarActionScan: function DLTB_onToolbarActionScan(sType, aArgs, p_obj)
      {
	
    	  var actionUrl = Alfresco.constants.PROXY_URI + "/scanner/send-doc-to-scan?noderef="+this.doclistMetadata.parent.nodeRef
    	  var scannerDialog = new Alfresco.module.SimpleDialog(this.id + "-scannerDialog");
	
    	  scannerDialog.setOptions({
    		  width: "30em",
    		  templateUrl: Alfresco.constants.URL_SERVICECONTEXT + "modules/select-scanner",				
    		  firstFocus: scannerDialog.id + "-select-scanner",
    		  doSetupFormsValidation: {
    			  fn: doSetupFormsValidation,
    			  scope: this
    		  },
    		  actionUrl : Alfresco.constants.PROXY_URI + "/scanner/send-doc-to-scan?noderef="+this.doclistMetadata.parent.nodeRef,
    		  onSuccess: {
    			  fn: function DLTB_onToolbarActionScan_success(response)
    		  	  {
    				  YAHOO.Bubbling.fire("metadataRefresh",
    				  {
    					  highlightFile: this.doclistMetadata.parent.displayName
    				  });
    				  Alfresco.util.PopupManager.displayMessage(
    			      	  {
    			    	  text: this.msg("message.scan.success")
    			      	  });
    		  	  },
              	  scope: this    			
    		  },		
    		  onFailure: {
    			  message : this.msg("message.scan.failure")
    		  }
    	  }).show();
      },
